 ...
 *   NOTE: the swc file's parent ids are all -1 for all nodes.  They will be re-connected.
 *
 ...
 *
 * 2017-3-10:  Take the image file from the ano file
 *
 * Usage ./vaa3d64 -x AT_trace_combiner -f combine_swc -i input.ano -p 20
 *
 * Todo:
 *       the ratio of meanshifting needs to be passed in as a parameter since it is sensitive
 *
 */
...
bool ATTraceCombinerPlugin::dofunc(const QString & func_name, const V3DPluginArgList & input, V3DPluginArgList & output, V3DPluginCallback2 & callback,  QWidget * parent)
{
	...
	if (func_name == tr("combine_swc"))
	{
        ...
        //get the raw image file from the ANO file
        P_ObjectFileType linker_object;
        loadAnoFile(PARA.inano_file,linker_object); //should succeed since it was loaded once before
        PARA.inimg_file = linker_object.raw_image_file_list[0];
